...
// tyrannosaurus, carnivorous, 7000kg, 12m, period
const tyrannosaurus = {
     name: 'tyrannosaurus',
   diet: 'carnivorous',
   weight: '7000kg',
...
const universities = [];
 ...
 graduates.forEach(function (uname) {
  universities.push(`${uname.university}`);
 });
...
const contactInfo = [];
graduates.forEach(function (uname) {
  contactInfo.push(`${uname.first_name} ${uname.email}`);
 });
...
using the .reduce() method. Remember the reduce method takes two arguments: a callback (which itself takes two args), and an initial value for the count.
